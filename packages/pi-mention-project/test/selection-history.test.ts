...
test("stateless ranking does not load persisted selection history", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "pi-mention-project-lazy-history-"));
...
test("createSelectionHistory lazily persists and reloads selections", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "pi-mention-project-history-"));
    const filePath = path.join(directory, "selections.json");
    try {
        const history = createSelectionHistory({ filePath, now: () => NOW });
        history.recordSelection("pi-tweaks");
        history.recordSelection("pi-tweaks");
        history.recordSelection("other-project");
        await history.flush();
        ...
        assert.deepEqual(stored, {
            version: 1,
            selections: {
                "pi-tweaks": { count: 2, lastSelectedAt: NOW },
                "other-project": { count: 1, lastSelectedAt: NOW },
            },
        });
        ...
        assert.deepEqual(
            await rankWithSelectionHistory(
                ["other-project", "pi-tweaks"],
                String,
                settings("frequent"),
                reloaded,
            ),
            ["pi-tweaks", "other-project"],
        );
    } finally {
        await rm(directory, { recursive: true, force: true });
    }
